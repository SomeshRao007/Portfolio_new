...
import { ChevronDownIcon, TrashIcon, PlusIcon, ArrowUturnLeftIcon, ArrowDownTrayIcon, ArrowUpOnSquareIcon } from '@heroicons/react/24/solid';
...
const AdminSection: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => {
  ...
  return (
    <div className="mb-4 border border-slate-300 rounded-lg">
      <button
        className="w-full flex justify-between items-center p-4 bg-slate-200 hover:bg-slate-300 rounded-t-lg"
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-4 sm:p-6 lg:p-8">
      <div className="max-w-5xl mx-auto">
        <header className="flex flex-wrap justify-between items-center gap-4 mb-8 bg-white p-4 rounded-lg shadow-md">
        ...
        <main>
            <AdminSection title="Hero & Personal Info">
                <div className="space-y-4">
                    ...
                    <input type="text" value={data.personalInfo.name} onChange={e => handlePersonalInfoChange('name', e.target.value)} className="mt-1 block w-full rounded-md border-slate-300 shadow-sm"/>
                    <label className="block text-sm font-medium text-slate-700">Title</label>
                    <input type="text" value={data.personalInfo.title} onChange={e => handlePersonalInfoChange('title', e.target.value)} className="mt-1 block w-full rounded-md border-slate-300 shadow-sm"/>
                    <label className="block text-sm font-medium text-slate-700">Bio</label>
                    <textarea value={data.personalInfo.bio} onChange={e => handlePersonalInfoChange('bio', e.target.value)} rows={4} className="mt-1 block w-full rounded-md border-slate-300 shadow-sm"/>
                    ...
                    <label className="block text-sm font-medium text-slate-700">Formspree Endpoint</label>
                    <input type="text" value={data.personalInfo.formspreeEndpoint} onChange={e => handlePersonalInfoChange('formspreeEndpoint', e.target.value)} className="mt-1 block w-full rounded-md border-slate-300 shadow-sm"/>
                    <h4 className="font-semibold pt-4">Social Links</h4>
                    {data.personalInfo.socialLinks.map((link, index) => (
                        <div key={index} className="grid grid-cols-2 gap-4">
                            <input type="text" placeholder="Name (e.g., GitHub)" value={link.name} onChange={e => handleSocialLinkChange(index, 'name', e.target.value)} className="block w-full rounded-md border-slate-300 shadow-sm"/>
                            <input type="text" placeholder="URL" value={link.url} onChange={e => handleSocialLinkChange(index, 'url', e.target.value)} className="block w-full rounded-md border-slate-300 shadow-sm"/>
                        </div>
                    ))}
                </div>
            </AdminSection>
            ...
            <AdminSection title="Skills">
                {data.skills.map((category, catIndex) => (
                    <div key={catIndex} className="bg-slate-50 p-4 rounded-lg mb-4 border border-slate-200">
                        <div className="flex items-center justify-between mb-2">
                            <input type="text" value={category.name} onChange={e => handleCategoryNameChange(catIndex, e.target.value)} className="text-lg font-semibold flex-grow rounded-md border-slate-300 shadow-sm"/>
                ...
                <AdminSection key={key} title={title}>
                    <div className="space-y-6">
                        {(data[key as keyof typeof data] as any[]).map((item, index) => (
                            <div key={index} className="space-y-2 bg-slate-50 p-4 rounded-lg relative">
            ...
            <AdminSection title="Stats">
                <div className="space-y-4">
                     {data.stats.map((stat, index) => (
                        <div key={index} className="grid grid-cols-2 gap-4">
                            <input type="text" placeholder="Label" value={stat.label} onChange={e => handleItemChange( 'stats', index, 'label', e.target.value)} className="block w-full rounded-md border-slate-300 shadow-sm"/>
                            <input type="number" placeholder="Value" value={stat.value} onChange={e => handleItemChange( 'stats', index, 'value', Number(e.target.value))} className="block w-full rounded-md border-slate-300 shadow-sm"/>
                        </div>
                    ))}
                </div>
            </AdminSection>
